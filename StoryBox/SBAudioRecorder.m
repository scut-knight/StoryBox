...
//        }
//        
//        NSString * fullPath = [NSString stringWithFormat:@"%@/%@.caf",fullDir,soundName];
//        
//        tempFilePath = [NSURL URLWithString:[fullPath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
//        
//        tempFilePath = [NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingPathComponent:
//                    [NSString stringWithFormat: @"%@.%@",@"wangshuo",@"caf"]]];
//        NSLog(@"url:%@",tempFilePath);

        recorder = [[AVAudioRecorder alloc] initWithURL:[self formSoundFileURL] settings:setting error:nil];
...
-(NSURL *)formSoundFileURL
{
    ...
    NSString * fullPath = [NSString stringWithFormat:@"%@/%i.caf",fullDir,recordNumber];
...
{
    NSString *soundFile = [NSString stringWithFormat:@"%i.caf",recordNumber];
