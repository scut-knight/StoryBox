...
-(void)scaleToSmall:(float)dis
{
    [UIView animateWithDuration:0.6 animations:^(void)
     {
         self.scrollView.transform=CGAffineTransformMakeScale(_scale, _scale);
    ...
    [self.scrollView bringSubviewToFront:imgv];
    [self.scrollView bringSubviewToFront:tV];
...
-(void)adjustALLwith_x:(float)d_x with_y:(float)d_y;
{
    ...
    float ff=self.scrollView.contentOffset.y;
    float offset_max=self.scrollView.contentSize.height-self.scrollView.frame.size.height;
...
-(void)detectBoundWithImagev:(UIImageView *) _view withTexv:(UIView*)_t withY:(float)_d_y
{
    ...
    float x1 = self.scrollView.frame.origin.x;
    float y1 = self.scrollView.frame.origin.y;
...
//    printf("curent=%d",current_index);
    ...
    float y_compare=(imgv2.center.y-self.scrollView.contentOffset.y)*_scale+self.scrollView.frame.origin.y;
    ...
    {//5
        ...
        {//存在
            ...
            {
                printf("上~~~上");
                _p=CGPointMake(self.scrollView.center.x, imgv3.frame.origin.y-8-imgv2.frame.size.height/2);
            }
            else
            {
                printf("下~~~下");
                _p=CGPointMake(self.scrollView.center.x, imgv3.frame.origin.y+imgv3.frame.size.height+8+imgv2.frame.size.height/2);
            }
        ...
        else
        {
            ...
            {//上
                 printf("上~上");
                 _p=CGPointMake(self.scrollView.center.x, self.scrollView.contentSize.height-imgv2.frame.size.height/2);
            }
            else
            {
                 printf("下~下");
                _p=CGPointMake(self.scrollView.center.x,imgv2.frame.size.height/2);
            }
...
//        printf("下");
    ...
    {
        CGPoint _p=CGPointMake(self.scrollView.contentOffset.x, self.scrollView.contentOffset.y+_d_move);//
        [self.scrollView setContentOffset:_p ];
    ...
    float  max_offset=self.scrollView.contentSize.height-self.scrollView.frame.size.height+_v.frame.size.height;
    if (self.scrollView.contentOffset.y<-_added||overBound==NO||self.scrollView.contentOffset.y>max_offset) {
//        printf("停止");
        [tm invalidate];
    }
}
...
-(void)changePaView:(UIView*)v withT:(UIView*)t
{
     ...
     float _y=v.center.y*_scale -self.scrollView.contentOffset.y*_scale +self.scrollView.frame.origin.y;
